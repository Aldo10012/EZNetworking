...
final class FileUploaderTaskCancellationTests {
    ...
    @Test("test upload when parent task is cancelled returns empty stream")
    func upload_parentTaskCancelled() async {
        let mockURLSession = MockFileUploaderURLSession()
        let session = MockSession(urlSession: mockURLSession, delegate: SessionDelegate())
        let sut = FileUploader(fileURL: mockFileURL, request: mockRequest, session: session)

        let eventsTask = Task {
            // Spin until this task is cancelled before calling upload
